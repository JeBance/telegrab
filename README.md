# 🤖 Telegrab — Архиватор сообщений Telegram

[![Python](https://img.shields.io/badge/Python-3.8%2B-blue.svg)](https://python.org)
[![FastAPI](https://img.shields.io/badge/FastAPI-0.104%2B-green.svg)](https://fastapi.tiangolo.com)
[![Telethon](https://img.shields.io/badge/Telethon-1.34%2B-green.svg)](https://docs.telethon.dev)
[![License](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Docker](https://img.shields.io/badge/Docker-ready-blue.svg)](https://docker.com)

**Telegrab** — это мощный инструмент для автоматического архивирования сообщений из Telegram-чатов в локальную базу данных с современным REST API, WebSocket для уведомлений в реальном времени и готовыми конфигурациями для production-развёртывания.

---

## 📋 Оглавление

- [Возможности](#-возможности)
- [Быстрый старт](#-быстрый-старт)
- [Установка на сервер](#-установка-на-сервер)
- [Получение API ключей Telegram](#-получение-api-ключей-telegram)
- [Конфигурация](#-конфигурация)
- [API Документация](#-api-документация)
- [WebSocket](#-websocket)
- [База данных](#-база-данных)
- [Безопасность](#-безопасность)
- [Production развёртывание](#-production-развёртывание)
- [Автообновление](#-автообновление)
- [Мониторинг и логи](#-мониторинг-и-логи)
- [Устранение проблем](#-устранение-проблем)
- [Структура проекта](#-структура-проекта)
- [Вклад в проект](#-вклад-в-проект)
- [Лицензия](#-лицензия)

---

## ✨ Возможности

| Функция | Описание |
|---------|----------|
| 🔐 **Аутентификация** | Защита API ключами через заголовок `X-API-Key` |
| 🔄 **WebSocket** | Push-уведомления о новых сообщениях в реальном времени |
| 📚 **Загрузка истории** | Поэтапная загрузка с соблюдением лимитов Telegram API |
| 🔍 **Поиск** | Полнотекстовый поиск по всем сохранённым сообщениям |
| 📊 **Статистика** | Подробная статистика по чатам и сообщениям |
| 🚀 **Production ready** | Docker, systemd, nginx — всё для работы на сервере |
| 🌐 **REST API** | Полноценный HTTP API с автодокументацией (Swagger) |
| 🤖 **UserBot режим** | Работает как обычный пользователь, не требует добавления ботов |
| ⚡ **Очередь задач** | Фоновая обработка задач с rate limiting |
| 📥 **Автозагрузка** | Автоматическая загрузка истории и пропущенных сообщений |
| 🔄 **Автообновление** | Автоматическое обновление из GitHub (systemd timer + webhook) |

---

## 🚀 Быстрый старт

### Требования

- **Python 3.8+** (для локального запуска)
- **Docker + Docker Compose** (рекомендуется для сервера)
- **Аккаунт Telegram** с номером телефона
- **API ключи Telegram** (API_ID и API_HASH)

### Вариант 1: Docker (рекомендуется)

```bash
# 1. Клонирование репозитория
git clone https://github.com/JeBance/telegrab.git
cd telegrab

# 2. Копирование конфигурации
cp .env.example .env

# 3. Редактирование .env (обязательно укажите API_ID, API_HASH, PHONE)
nano .env

# 4. Запуск через Docker Compose
docker-compose up -d

# 5. Проверка логов
docker-compose logs -f telegrab

# 6. Остановка
docker-compose down
```

### Вариант 2: Локальная разработка

```bash
# 1. Установка зависимостей
pip install -r requirements.txt

# 2. Копирование конфигурации
cp .env.example .env

# 3. Редактирование .env
nano .env

# 4. Запуск приложения
python telegrab.py

# 5. Первый запуск запросит код из SMS
#    После успешной авторизации сессия сохранится в файл .session
```

### Вариант 3: Автоматическая установка на сервер

```bash
# 1. Клонирование и запуск установщика
git clone https://github.com/JeBance/telegrab.git
cd telegrab
chmod +x install.sh
./install.sh

# 2. Редактирование конфигурации
sudo nano /opt/telegrab/.env

# 3. Запуск сервиса
sudo systemctl start telegrab
sudo systemctl enable telegrab

# 4. Проверка статуса
sudo systemctl status telegrab

# 5. Просмотр логов
journalctl -u telegrab -f
```

---

## 🛠️ Установка на сервер (подробно)

### Подготовка сервера (Ubuntu 22.04 LTS)

```bash
# Обновление системы
sudo apt update && sudo apt upgrade -y

# Установка Docker
curl -fsSL https://get.docker.com -o get-docker.sh
sudo sh get-docker.sh

# Добавление пользователя в группу docker
sudo usermod -aG docker $USER
newgrp docker

# Установка Docker Compose
sudo apt install -y docker-compose

# Проверка установки
docker --version
docker-compose --version
```

### Развёртывание Telegrab

```bash
# Создание директории
sudo mkdir -p /opt/telegrab
sudo chown $USER:$USER /opt/telegrab
cd /opt/telegrab

# Клонирование репозитория
git clone https://github.com/JeBance/telegrab.git .

# Копирование конфигурации
cp .env.example .env

# Редактирование конфигурации
nano .env
```

### Настройка .env

```ini
# === ОБЯЗАТЕЛЬНЫЕ ПАРАМЕТРЫ ===
# Получить на https://my.telegram.org
API_ID=12345678
API_HASH=abcdef1234567890abcdef1234567890
PHONE=+79991234567

# === ОПЦИОНАЛЬНЫЕ ПАРАМЕТРЫ ===
# API ключ (генерируется автоматически, можно указать свой)
# API_KEY=tg_your_secret_key_here

# Порт API (по умолчанию 3000)
API_PORT=3000

# Автозагрузка истории при старте
AUTO_LOAD_HISTORY=true
HISTORY_LIMIT_PER_CHAT=200
MAX_CHATS_TO_LOAD=20

# Автодогрузка пропущенных сообщений
AUTO_LOAD_MISSED=true
MISSED_LIMIT_PER_CHAT=500
MISSED_DAYS_LIMIT=7

# Ограничение скорости (для избежания блокировок Telegram)
REQUESTS_PER_SECOND=1
MESSAGES_PER_REQUEST=100
```

### Запуск

```bash
# Через Docker Compose
docker-compose up -d

# Проверка статуса
docker-compose ps

# Просмотр логов
docker-compose logs -f
```

---

## 🔑 Получение API ключей Telegram

1. **Перейдите на сайт разработки Telegram:**
   - Откройте https://my.telegram.org
   - Войдите под своим номером телефона

2. **Создайте приложение:**
   - Выберите **"API Development Tools"**
   - Заполните форму:
     - **App title**: Любое название (например, `Telegrab`)
     - **Short name**: Короткое имя (например, `telegrab`)
     - **URL**: Можно оставить пустым
     - **Platform**: Выберите `Desktop`

3. **Получите ключи:**
   - Скопируйте **API ID** (число)
   - Скопируйте **API Hash** (32 символа)
   - Вставьте значения в файл `.env`

⚠️ **Важно:** Никогда не передавайте API ключи третьим лицам и не коммитьте файл `.env` в публичный репозиторий!

---

## ⚙️ Конфигурация

### Обязательные параметры

| Параметр | Описание | Пример |
|----------|----------|--------|
| `API_ID` | ID приложения Telegram | `12345678` |
| `API_HASH` | Хэш приложения Telegram | `abcdef1234567890...` |
| `PHONE` | Номер телефона для авторизации | `+79991234567` |

### Параметры API

| Параметр | По умолчанию | Описание |
|----------|--------------|----------|
| `API_KEY` | авто | Ключ для аутентификации API |
| `API_PORT` | `3000` | Порт HTTP сервера |

### Настройки загрузки истории

| Параметр | По умолчанию | Описание |
|----------|--------------|----------|
| `AUTO_LOAD_HISTORY` | `true` | Загружать историю при старте |
| `HISTORY_LIMIT_PER_CHAT` | `200` | Сообщений на чат за один раз |
| `MAX_CHATS_TO_LOAD` | `20` | Максимум чатов для загрузки |

### Настройки пропущенных сообщений

| Параметр | По умолчанию | Описание |
|----------|--------------|----------|
| `AUTO_LOAD_MISSED` | `true` | Догружать пропущенные при старте |
| `MISSED_LIMIT_PER_CHAT` | `500` | Максимум пропущенных на чат |
| `MISSED_DAYS_LIMIT` | `7` | Максимум дней для догрузки |

### Rate limiting (ограничение скорости)

| Параметр | По умолчанию | Описание |
|----------|--------------|----------|
| `REQUESTS_PER_SECOND` | `1` | Запросов к Telegram API в секунду |
| `MESSAGES_PER_REQUEST` | `100` | Сообщений за один запрос |

⚠️ **Важно:** Не устанавливайте слишком высокие значения rate limiting — это может привести к временной блокировке со стороны Telegram!

---

## 🌐 API Документация

### Базовый URL

```
http://localhost:3000
```

При использовании nginx с HTTPS:
```
https://your-domain.com
```

### Аутентификация

Все endpoints (кроме `/`, `/health` и `/docs`) требуют API ключ в заголовке:

```
X-API-Key: ваш_api_key
```

API ключ генерируется автоматически при первом запуске и сохраняется в файле `.env`.

### Swagger UI

Интерактивная документация доступна по адресу:
```
http://localhost:3000/docs
```

---

### HTTP Endpoints

#### Общая информация

| Метод | Endpoint | Описание | Auth |
|-------|----------|----------|------|
| `GET` | `/` | Информация о сервисе | ❌ |
| `GET` | `/health` | Проверка работоспособности | ❌ |
| `GET` | `/docs` | Swagger UI документация | ❌ |

#### Сообщения и чаты

| Метод | Endpoint | Описание | Auth |
|-------|----------|----------|------|
| `GET` | `/stats` | Статистика архива | ✅ |
| `GET` | `/chats` | Список всех чатов | ✅ |
| `GET` | `/messages` | Сообщения с фильтрацией | ✅ |
| `GET` | `/search?q=...` | Поиск по сообщениям | ✅ |

#### Задачи и загрузка

| Метод | Endpoint | Описание | Auth |
|-------|----------|----------|------|
| `POST` | `/load` | Загрузить историю чата | ✅ |
| `GET` | `/task/{id}` | Статус задачи загрузки | ✅ |
| `GET` | `/queue` | Статус очереди задач | ✅ |
| `GET` | `/chat_status/{id}` | Статус загрузки чата | ✅ |
| `POST` | `/load_missed_all` | Догрузить пропущенные для всех | ✅ |

#### WebSocket

| Метод | Endpoint | Описание | Auth |
|-------|----------|----------|------|
| `WS` | `/ws` | WebSocket для real-time | ❌ |

---

### Примеры запросов

#### Получить статистику

```bash
curl -H "X-API-Key: ваш_ключ" \
  http://localhost:3000/stats
```

**Ответ:**
```json
{
  "total_messages": 15420,
  "total_chats": 12,
  "fully_loaded_chats": 5,
  "last_saved": "2024-02-22T10:30:00"
}
```

#### Получить список чатов

```bash
curl -H "X-API-Key: ваш_ключ" \
  http://localhost:3000/chats
```

**Ответ:**
```json
{
  "count": 12,
  "chats": [
    {
      "chat_id": -1001234567890,
      "chat_title": "Crypto News",
      "last_message": "2024-02-22T10:25:00",
      "message_count": 5420,
      "fully_loaded": 1,
      "total_loaded": 5420
    }
  ]
}
```

#### Получить сообщения

```bash
# Все сообщения
curl -H "X-API-Key: ваш_ключ" \
  "http://localhost:3000/messages?limit=50&offset=0"

# Сообщения конкретного чата
curl -H "X-API-Key: ваш_ключ" \
  "http://localhost:3000/messages?chat_id=-1001234567890&limit=100"

# С поиском
curl -H "X-API-Key: ваш_ключ" \
  "http://localhost:3000/messages?search=биткоин&limit=50"
```

#### Поиск сообщений

```bash
curl -H "X-API-Key: ваш_ключ" \
  "http://localhost:3000/search?q=биткоин&limit=100"
```

**Ответ:**
```json
{
  "query": "биткоин",
  "count": 42,
  "results": [
    {
      "message_id": 12345,
      "chat_id": -1001234567890,
      "chat_title": "Crypto News",
      "text": "Биткоин пробил новый максимум...",
      "sender_name": "Admin",
      "message_date": "2024-02-22T09:15:00",
      "saved_at": "2024-02-22T09:15:30"
    }
  ]
}
```

#### Загрузить историю чата

```bash
# Загрузка истории конкретного чата
curl -X POST \
  -H "X-API-Key: ваш_ключ" \
  -H "Content-Type: application/json" \
  -d '{"chat_id": "@durov", "limit": 100}' \
  http://localhost:3000/load

# Вступить в чат и загрузить историю
curl -X POST \
  -H "X-API-Key: ваш_ключ" \
  -H "Content-Type: application/json" \
  -d '{"chat_id": "@crypto_channel", "join": true}' \
  http://localhost:3000/load

# Догрузить пропущенные сообщения
curl -X POST \
  -H "X-API-Key: ваш_ключ" \
  -H "Content-Type: application/json" \
  -d '{"chat_id": -1001234567890, "missed": true}' \
  http://localhost:3000/load
```

**Ответ:**
```json
{
  "task_id": "a1b2c3d4",
  "status": "queued",
  "message": "Задача добавлена в очередь",
  "queue_position": 1
}
```

#### Проверить статус задачи

```bash
curl -H "X-API-Key: ваш_ключ" \
  http://localhost:3000/task/a1b2c3d4
```

**Ответ:**
```json
{
  "id": "a1b2c3d4",
  "type": "load_history",
  "status": "completed",
  "created_at": "2024-02-22T10:00:00",
  "started_at": "2024-02-22T10:00:01",
  "completed_at": "2024-02-22T10:05:00",
  "result": {
    "chat_id": -1001234567890,
    "chat_title": "Crypto News",
    "new_messages": 500,
    "fully_loaded": true
  }
}
```

#### Статус очереди задач

```bash
curl -H "X-API-Key: ваш_ключ" \
  http://localhost:3000/queue
```

**Ответ:**
```json
{
  "size": 2,
  "processing": true,
  "requests_per_second": 1
}
```

#### Статус загрузки чата

```bash
curl -H "X-API-Key: ваш_ключ" \
  http://localhost:3000/chat_status/-1001234567890
```

**Ответ:**
```json
{
  "chat_id": -1001234567890,
  "last_loaded_id": 12345,
  "last_message_date": "2024-02-22T10:00:00",
  "total_loaded": 5420,
  "fully_loaded": 1,
  "last_loading_date": "2024-02-22T10:00:00",
  "last_saved_message_date": "2024-02-22T10:25:00"
}
```

#### Догрузить пропущенные для всех чатов

```bash
curl -X POST \
  -H "X-API-Key: ваш_ключ" \
  http://localhost:3000/load_missed_all
```

**Ответ:**
```json
{
  "task_ids": ["a1b2c3d4", "e5f6g7h8"],
  "message": "Задачи созданы для 2 чатов",
  "total_chats": 2
}
```

---

## 🔌 WebSocket

WebSocket endpoint позволяет получать уведомления о событиях в реальном времени.

### Подключение

```
ws://localhost:3000/ws
```

### Типы событий

| Тип события | Описание | Данные |
|-------------|----------|--------|
| `new_message` | Новое сообщение сохранено | `message` |
| `task_completed` | Задача завершена | `task` |
| `loading_progress` | Прогресс загрузки | `chat_id`, `loaded`, `total` |
| `chat_loaded` | Чат полностью загружен | `chat_id`, `new_messages` |
| `missed_loaded` | Пропущенные загружены | `chat_id`, `count` |
| `pong` | Ответ на ping | — |

### Пример на JavaScript

```javascript
const ws = new WebSocket('ws://localhost:3000/ws');

ws.onopen = () => {
  console.log('WebSocket подключён');
  
  // Отправляем ping для проверки соединения
  ws.send(JSON.stringify({ type: 'ping' }));
};

ws.onmessage = (event) => {
  const data = JSON.parse(event.data);
  
  switch (data.type) {
    case 'new_message':
      console.log('📩 Новое сообщение:', data.message);
      console.log('   Чат:', data.message.chat_title);
      console.log('   Текст:', data.message.text);
      break;
      
    case 'task_completed':
      console.log('✅ Задача завершена:', data.task);
      break;
      
    case 'loading_progress':
      console.log('📥 Загрузка:', data.loaded, '/', data.total);
      break;
      
    case 'chat_loaded':
      console.log('📚 Чат загружен:', data.chat_title);
      console.log('   Новых сообщений:', data.new_messages);
      break;
      
    case 'missed_loaded':
      console.log('🔍 Пропущенные загружены:', data.count);
      break;
      
    case 'pong':
      console.log('🏓 Pong получен');
      break;
  }
};

ws.onerror = (error) => {
  console.error('Ошибка WebSocket:', error);
};

ws.onclose = () => {
  console.log('WebSocket отключён. Переподключение...');
  setTimeout(() => location.reload(), 3000);
};
```

### Пример на Python

```python
import asyncio
import websockets
import json

async def listen():
    uri = "ws://localhost:3000/ws"
    async with websockets.connect(uri) as websocket:
        print("WebSocket подключён")
        
        while True:
            message = await websocket.recv()
            data = json.loads(message)
            
            if data['type'] == 'new_message':
                msg = data['message']
                print(f"📩 [{msg['chat_title']}] {msg['sender_name']}: {msg['text'][:50]}...")
            
            elif data['type'] == 'task_completed':
                print(f"✅ Задача {data['task']['id']} завершена")

asyncio.run(listen())
```

---

## 🗄️ База данных

### Расположение

```
data/telegrab.db
```

### Структура таблиц

#### Таблица `messages`

| Колонка | Тип | Описание |
|---------|-----|----------|
| `id` | INTEGER | Автоинкремент ID |
| `message_id` | INTEGER | ID сообщения в Telegram (уникальный) |
| `chat_id` | INTEGER | ID чата |
| `chat_title` | TEXT | Название чата |
| `text` | TEXT | Текст сообщения |
| `sender_name` | TEXT | Имя отправителя |
| `message_date` | TEXT | Дата сообщения (ISO 8601) |
| `saved_at` | TIMESTAMP | Когда сохранено (авто) |

#### Таблица `chat_loading_status`

| Колонка | Тип | Описание |
|---------|-----|----------|
| `chat_id` | INTEGER | ID чата (первичный ключ) |
| `last_loaded_id` | INTEGER | ID последнего загруженного сообщения |
| `last_message_date` | TEXT | Дата последнего сообщения |
| `total_loaded` | INTEGER | Всего загружено сообщений |
| `fully_loaded` | BOOLEAN | Полностью ли загружен чат |
| `last_loading_date` | TEXT | Дата последней загрузки |
| `created_at` | TIMESTAMP | Дата создания записи |

### Индексы

```sql
CREATE INDEX idx_chat ON messages(chat_id);
CREATE INDEX idx_date ON messages(message_date);
CREATE INDEX idx_saved_at ON messages(saved_at);
```

### Резервное копирование

```bash
# Создание бэкапа
cp data/telegrab.db data/telegrab.backup.$(date +%Y%m%d).db

# Восстановление из бэкапа
cp data/telegrab.backup.20240222.db data/telegrab.db
```

---

## 🔒 Безопасность

### API ключи

- API ключ генерируется автоматически при первом запуске
- Ключ хранится в файле `.env`
- **Никогда не коммитьте `.env` в git!** (файл добавлен в `.gitignore`)

### Смена API ключа

```bash
# Сгенерировать новый ключ
NEW_KEY="tg_$(openssl rand -hex 16)"
sed -i "s/^API_KEY=.*/API_KEY=$NEW_KEY/" .env

# Перезапустить приложение
docker-compose restart telegrab
```

### HTTPS (nginx)

Для production используйте nginx с SSL:

```bash
# Установка Certbot
sudo apt install -y certbot python3-certbot-nginx

# Получение сертификата
sudo certbot --nginx -d your-domain.com

# Копирование сертификатов для Docker
mkdir -p ssl
sudo cp /etc/letsencrypt/live/your-domain.com/fullchain.pem ssl/
sudo cp /etc/letsencrypt/live/your-domain.com/privkey.pem ssl/

# Раскомментируйте nginx в docker-compose.yml
nano docker-compose.yml

# Перезапуск
docker-compose up -d nginx
```

### Firewall

```bash
# Настройка UFW
sudo ufw allow 22/tcp    # SSH
sudo ufw allow 443/tcp   # HTTPS (если используете nginx)
sudo ufw deny 3000/tcp   # Закрыть прямой доступ к API
sudo ufw enable

# Проверка
sudo ufw status
```

### Безопасность сервера

```bash
# Обновление системы
sudo apt update && sudo apt upgrade -y

# Установка fail2ban
sudo apt install -y fail2ban
sudo systemctl enable fail2ban
sudo systemctl start fail2ban

# Отключение root login
sudo nano /etc/ssh/sshd_config
# PermitRootLogin no
# PasswordAuthentication no

sudo systemctl restart sshd
```

---

## 🏭 Production развёртывание

### Минимальные требования к серверу

| Ресурс | Требование |
|--------|------------|
| CPU | 1 ядро |
| RAM | 512 MB |
| Disk | 5 GB |
| OS | Ubuntu 22.04 LTS |

### Рекомендуемая конфигурация

| Ресурс | Требование |
|--------|------------|
| CPU | 2 ядра |
| RAM | 1 GB |
| Disk | 20 GB SSD |
| OS | Ubuntu 22.04 LTS |

### Пошаговая инструкция

#### 1. Подготовка сервера

```bash
# Обновление
sudo apt update && sudo apt upgrade -y

# Установка Docker
curl -fsSL https://get.docker.com -o get-docker.sh
sudo sh get-docker.sh

# Установка Docker Compose
sudo apt install -y docker-compose

# Настройка автозапуска Docker
sudo systemctl enable docker
sudo systemctl start docker
```

#### 2. Установка Telegrab

```bash
# Создание директории
sudo mkdir -p /opt/telegrab
sudo chown $USER:$USER /opt/telegrab
cd /opt/telegrab

# Клонирование
git clone https://github.com/JeBance/telegrab.git .

# Настройка
cp .env.example .env
nano .env  # укажите API_ID, API_HASH, PHONE
```

#### 3. Запуск

```bash
# Старт
docker-compose up -d

# Проверка
docker-compose ps
docker-compose logs -f
```

#### 4. Настройка nginx (опционально)

```bash
# Создание SSL директории
mkdir ssl

# Получение сертификата
sudo apt install -y certbot
sudo certbot certonly --standalone -d your-domain.com

# Копирование
sudo cp /etc/letsencrypt/live/your-domain.com/fullchain.pem ssl/
sudo cp /etc/letsencrypt/live/your-domain.com/privkey.pem ssl/

# Редактирование docker-compose.yml (раскомментировать nginx)
nano docker-compose.yml

# Перезапуск
docker-compose up -d
```

#### 5. Мониторинг

```bash
# Создание скрипта мониторинга
cat > /usr/local/bin/telegrab-check.sh << 'EOF'
#!/bin/bash
if ! docker-compose ps | grep -q "Up"; then
    echo "Telegrab не работает! Перезапуск..."
    cd /opt/telegrab
    docker-compose restart
    echo "Telegrab перезапущен" | mail -s "Telegrab Alert" admin@example.com
fi
EOF

chmod +x /usr/local/bin/telegrab-check.sh

# Добавление в crontab
echo "*/5 * * * * /usr/local/bin/telegrab-check.sh" | crontab -
```

---

## 🔄 Автообновление

Telegrab поддерживает автоматическое обновление из GitHub двумя способами:

### Способ 1: Периодическое обновление (systemd timer)

Проверка обновлений раз в сутки в 3:00 ночи:

```bash
# 1. Скопируйте скрипты
sudo mkdir -p /opt/telegrab/scripts
sudo cp scripts/*.sh /opt/telegrab/scripts/
sudo cp scripts/*.service /opt/telegrab/scripts/
sudo cp scripts/*.timer /opt/telegrab/scripts/
sudo chmod +x /opt/telegrab/scripts/*.sh

# 2. Установите systemd unit файлы
sudo cp /opt/telegrab/scripts/telegrab-update.service /etc/systemd/system/
sudo cp /opt/telegrab/scripts/telegrab-update.timer /etc/systemd/system/
sudo systemctl daemon-reload

# 3. Включите таймер
sudo systemctl enable telegrab-update.timer
sudo systemctl start telegrab-update.timer

# 4. Проверьте статус
sudo systemctl list-timers | grep telegrab
```

### Способ 2: Мгновенное обновление (GitHub webhook)

Обновление сразу после push в репозиторий:

**Настройка на GitHub:**
1. Settings → Webhooks → Add webhook
2. Payload URL: `http://your-server-ip:8080`
3. Secret: придумайте секретную строку
4. Events: Just the push event

**Настройка на сервере:**
```bash
# 1. Отредактируйте webhook-server.sh
sudo nano /opt/telegrab/scripts/webhook-server.sh
# Укажите SECRET="ваш_секрет_из_github"

# 2. Установите сервис
sudo cp /opt/telegrab/scripts/telegrab-webhook.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable telegrab-webhook.service
sudo systemctl start telegrab-webhook.service
```

### Ручное обновление

```bash
sudo /opt/telegrab/scripts/auto-update.sh
```

### Логи обновлений

```bash
journalctl -u telegrab-update.service -f
```

📚 **Подробная документация:** [scripts/README.md](scripts/README.md)

---

## 📊 Мониторинг и логи

### Docker Compose логи

```bash
# Просмотр логов в реальном времени
docker-compose logs -f telegrab

# Последние 100 строк
docker-compose logs --tail=100 telegrab

# Логи за последние 24 часа
docker-compose logs --since=24h telegrab
```

### systemd логи (при установке через install.sh)

```bash
# Просмотр логов
journalctl -u telegrab -f

# Логи за сегодня
journalctl -u telegrab --since today

# Поиск по тексту
journalctl -u telegrab | grep "ошибка"
```

### Health check

```bash
# Проверка работоспособности
curl http://localhost:3000/health

# Проверка с API ключом
curl -H "X-API-Key: ваш_ключ" http://localhost:3000/stats
```

### Метрики для мониторинга

| Метрика | Endpoint | Описание |
|---------|----------|----------|
| Статус сервиса | `/health` | Работоспособность |
| Количество сообщений | `/stats` | total_messages |
| Количество чатов | `/stats` | total_chats |
| Статус задач | `/queue` | size, processing |

---

## 🔧 Устранение проблем

### Частые ошибки

#### 1. "API_ID не установлен"

**Причина:** Файл `.env` не найден или не заполнен.

**Решение:**
```bash
cp .env.example .env
nano .env  # укажите API_ID, API_HASH, PHONE
docker-compose restart telegrab
```

#### 2. "Telethon не установлен"

**Причина:** Зависимости не установлены.

**Решение:**
```bash
pip install -r requirements.txt
# или
docker-compose build telegrab
docker-compose up -d telegrab
```

#### 3. "Потеряно соединение с Telegram"

**Причина:** Проблемы с интернетом или блокировка.

**Решение:**
- Проверьте подключение к интернету
- Увеличьте `connection_retries` в коде
- Попробуйте использовать прокси

#### 4. "Превышен лимит запросов Telegram"

**Причина:** Слишком высокие значения rate limiting.

**Решение:**
```ini
# Уменьшите в .env
REQUESTS_PER_SECOND=0.5
MESSAGES_PER_REQUEST=50
```

#### 5. "API ключ неверен"

**Причина:** Ключ изменился или не совпадает.

**Решение:**
```bash
# Проверьте ключ в .env
cat .env | grep API_KEY

# Используйте правильный ключ в запросах
curl -H "X-API-Key: правильный_ключ" ...
```

#### 6. "Port 3000 already in use"

**Причина:** Порт занят другим приложением.

**Решение:**
```bash
# Найти процесс
sudo lsof -i :3000

# Изменить порт в .env
API_PORT=3001

# Перезапустить
docker-compose restart telegrab
```

#### 7. "Session expired" или требуется повторная авторизация

**Причина:** Сессия Telegram устарела.

**Решение:**
```bash
# Удалить старую сессию
rm .session

# Перезапустить и ввести код из SMS
docker-compose restart telegrab
docker-compose logs -f telegrab
```

### Отладка

```bash
# Включить подробное логирование
docker-compose logs -f telegrab 2>&1 | tee telegrab.log

# Проверка конфигурации
docker-compose exec telegrab python -c "from api import CONFIG; print(CONFIG)"

# Проверка базы данных
docker-compose exec telegrab sqlite3 data/telegrab.db "SELECT COUNT(*) FROM messages;"
```

---

## 📁 Структура проекта

```
telegrab/
├── telegrab.py              # Точка входа, запуск приложения
├── api.py                   # FastAPI сервер, WebSocket, бизнес-логика
├── requirements.txt         # Python зависимости
├── .env.example            # Шаблон конфигурации
├── .env                    # Актуальная конфигурация (не коммитить!)
├── .session                # Сессия Telegram (не коммитить!)
├── .gitignore              # Git ignore правила
├── .dockerignore           # Docker ignore правила
├── Dockerfile              # Docker образ
├── docker-compose.yml      # Docker Compose конфигурация
├── telegrab.service        # systemd сервис
├── nginx.conf              # nginx конфигурация
├── install.sh              # Скрипт автоматической установки
├── README.md               # Документация
└── data/
    └── telegrab.db         # SQLite база данных
```

---

## 🤝 Вклад в проект

Приветствуются любые вклады в проект!

### Как внести вклад

1. **Fork** репозиторий
2. Создайте ветку для новой функции:
   ```bash
   git checkout -b feature/amazing-feature
   ```
3. Внесите изменения и закоммитьте:
   ```bash
   git commit -m "feat: добавить amazing feature"
   ```
4. Отправьте в репозиторий:
   ```bash
   git push origin feature/amazing-feature
   ```
5. Создайте **Pull Request**

### Соглашения по коммитам

- `feat:` — новая функция
- `fix:` — исправление ошибки
- `docs:` — документация
- `style:` — форматирование
- `refactor:` — рефакторинг
- `test:` — тесты
- `chore:` — прочее

### Баги и предложения

- 🐛 **Нашли баг?** Создайте [Issue](https://github.com/JeBance/telegrab/issues)
- 💡 **Есть идея?** Создайте [Discussion](https://github.com/JeBance/telegrab/discussions)

---

## 📄 Лицензия

Этот проект распространяется под лицензией **MIT License**.

См. файл [LICENSE](LICENSE) для подробностей.

---

## 📞 Контакты

- **GitHub:** [@JeBance](https://github.com/JeBance)
- **Issues:** [Сообщить о проблеме](https://github.com/JeBance/telegrab/issues)
- **Discussions:** [Обсуждение](https://github.com/JeBance/telegrab/discussions)

---

## ⭐ Поддержка

Если проект оказался полезным — поставьте звезду на GitHub! ⭐

Это поможет проекту развиваться и привлекать новых участников.

---

## 📝 Changelog

### v4.0 (2024)
- ✨ Переход на FastAPI + Uvicorn
- 🔐 Аутентификация через API ключи
- 🔌 WebSocket для real-time уведомлений
- 🐳 Docker и Docker Compose поддержка
- ⚙️ systemd сервис для автозапуска
- 🌐 nginx конфигурация для HTTPS
- 📚 Обновлённая документация

### v3.2 (2023)
- 🔍 Умная догрузка пропущенных сообщений
- 📊 Расширенная статистика
- ⚡ Очередь задач с rate limiting

### v3.0 (2023)
- 🌐 REST API
- 📚 Автосохранение истории чатов
- 🤖 UserBot режим работы

---

<div align="center">

**Сделано с ❤️ для архивирования Telegram**

[Наверх](#-telegrab---архиватор-сообщений-telegram)

</div>
