...
| `TELEGRAM_MODE` | `production` | Режим Telegram: `test` или `production` |

### 🧪 Тестовый режим

Для безопасной разработки и тестирования используйте тестовые сервера Telegram:
...
**Важно:**
- Тестовые аккаунты Telegram **не работают** с production серверами
- Получите тестовый номер на https://my.telegram.org/test
- Не используйте личный номер для тестирования
- Сессии тестовых и production аккаунтов **не совместимы**

**Сервера (начальные адреса для подключения):**
| Режим | DC ID | Сервер | Порт |
|-------|-------|--------|------|
| **Test** | 2 | 149.154.167.40 | 443 |
| **Production** | 2 | 149.154.167.50 | 443 |

> 💡 **Примечание:** После первого подключения Telethon автоматически получает актуальную конфигурацию всех DC серверов через `help.getConfig`. Указанные адреса используются только как начальная точка входа.
